...
export function InlineTaskEditor({
    ...
    async function loadAttachments() {
      ...
      await Promise.all(
        list
          .filter(
            (attachment) =>
              attachment.mime_type !== LINK_MIME &&
  ...
  const save = async (overrides: Partial<typeof form> = {}) => {
    ...
    const payload = {
      ...
      completed_at:
        merged.status === "done" ? (task.completed_at ?? new Date().toISOString()) : null,
    };
      ...
      <div
        className={
          compact
            ...
            : "max-h-[75vh] overflow-y-auto rounded-lg border bg-card p-3 shadow-md ring-2 ring-primary/40"
        }
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              ...
              <Select
                value={form.tag_id}
                onValueChange={(value) => setForm({ ...form, tag_id: value })}
              >
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(value) =>
                  setForm({ ...form, status: value === "none" ? null : (value as Task["status"]) })
                }
              >
                <SelectTrigger className={selectTriggerClass}>
                  <SelectValue />
            ...
            <>
              <div>
                ...
                <Select
                  ...
                  onValueChange={(value) => setForm({ ...form, column_id: value })}
                >
                  <SelectTrigger className={selectTriggerClass}>
                    <SelectValue />
                  </SelectTrigger>

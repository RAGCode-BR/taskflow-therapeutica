...
import { richHtmlToText } from "@/lib/sanitize-html";
...
export function ClientsIndexPage() {
  ...
  const [color, setColor] = useState("#5d6e3e");
  ...
  useEffect(() => {
    ...
    const loadAvatarUrls = async () => {
      ...
      const urlByPath = new Map(
        (data ?? []).map(
          (item: { path: string | null; signedUrl: string }) =>
    ...
    if (user && isOffline()) {
      if (edit) {
        await enqueueOfflineOperation({
          ...
          action: "update",
          entityId: edit.id,
          payload: { table: "clients", patch: clientData },
        });
        qc.setQueryData<Client[]>(["clients"], (current = []) =>
          current.map((client) => (client.id === edit.id ? { ...client, ...clientData } : client)),
        );
      } else {
        const client: Client = {
        ...
        await enqueueOfflineOperation({
          ...
          action: "create",
          entityId: client.id,
          payload: { table: "clients", record: { ...client, created_by: user.id } },
        });
  ...
  const setClientActive = async (client: Client, isActive: boolean) => {
    ...
    if (user && isOffline()) {
      await enqueueOfflineOperation({
        ...
        action: "update",
        entityId: client.id,
        payload: { table: "clients", patch: { is_active: isActive } },
      });
      qc.setQueryData<Client[]>(["clients"], (current = []) =>

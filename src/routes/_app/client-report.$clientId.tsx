...
import { escapeHtmlText, sanitizeRichHtml } from "@/lib/sanitize-html";
...
function ClientReportPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      setHtml(sanitizeRichHtml(r.html));
      setStats(r.stats);
    } catch (e) {
  ...
  const print = () => {
    ...
    if (!w) return;
    const safeClientName = escapeHtmlText(client?.name);
    const safeReportHtml = sanitizeRichHtml(html);
    w.document
      .write(`<!doctype html><html><head><meta charset="utf-8"/><title>Relatório — ${safeClientName}</title>
      <style>
              ...
              <div className="flex items-center gap-1">
                <Button
                  ...
                  disabled={
                    ...
                    selectedAssignees.length === assignableProfiles.length
                  }
                >
                  Selecionar todos
                </Button>

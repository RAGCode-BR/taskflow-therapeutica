...
function ImportAtaPage() {
  ...
  const defaultColumnId = useMemo(
    () =>
      columns.find((column) => column.name.trim().toLocaleLowerCase("pt-BR") === "a fazer")?.id ??
      columns[0]?.id ??
      null,
    [columns],
  );
    ...
    try {
      ...
      previewDocument.write(`<!doctype html><html><head><meta charset="utf-8" /><style>
          * { box-sizing: border-box; }
          ...
          .ata-pdf-content h2 { margin: 0 0 20px; color: #5d6e3e; font-size: 24px; line-height: 1.2; }
          .ata-pdf-content h3 { margin: 22px 0 9px; color: #5d6e3e; font-size: 16px; line-height: 1.25; }
          ...
          .ata-pdf-content th { background: #f1ede3; color: #5d6e3e; }
          ...
          <div class="ata-pdf-content">${sanitizeRichHtml(ataHtml)}</div>
        </main></body></html>`);
      ...
      pdf.save(
        `${
          (ataTitle || "ata-de-reuniao")
            .replace(/[^a-z0-9]+/gi, "-")
            .replace(/^-|-$/g, "")
            .toLowerCase() || "ata-de-reuniao"
        }.pdf`,
      );
    ...
    <div className="mx-auto w-full max-w-6xl p-4 md:p-6 space-y-4">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Gere uma <strong>ata formatada</strong> a partir de notas brutas e/ou extraia{" "}
            <strong>tarefas</strong> para o Kanban. Use os dois ou apenas um.
          </p>
        </div>
      </div>
      ...
      <Card className="p-4">
        <Tabs value={tab} onValueChange={(v) => setTab(v as "pdf" | "text")}>
          ...
          <TabsContent value="pdf" className="mt-3 space-y-3">
            <FileDropZone onFiles={(files) => setFile(files.item(0))} className="rounded-md">
      ...
      {ataHtml && (
        <Card className="p-4 space-y-3">
          ...
          <div className="text-xs text-muted-foreground">
            A criação de tarefas no Kanban é opcional e independente — use o botão "Extrair Tarefas"
            acima se quiser também transformar as ações em cards.
          </div>
        </Card>
      )}
        ...
        <Card className="p-4 space-y-3">
          <div className="flex items-center justify-between">
            <div className="text-sm font-medium">
              Tarefas sugeridas ({rows.filter((r) => r._selected).length}/{rows.length})
            </div>
          ...
          <div className="space-y-3">
            {rows.map((r) => (
              <div
                key={r._id}
                className={`rounded-md border p-3 space-y-2 ${r._selected ? "" : "opacity-50"}`}
              >
                <div className="flex items-start gap-2">
                  <Checkbox
                    checked={r._selected}
                    onCheckedChange={(c) => updateRow(r._id, { _selected: !!c })}
                    className="mt-1"
                  />
                  <div className="flex-1 space-y-2">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
                      <div>
                        <div className="text-[11px] text-muted-foreground mb-1">
                          Responsável{" "}
                          {r.assignee_name && !r.assignee_id ? (
                            <span className="text-amber-600">(sugestão: {r.assignee_name})</span>
                          ) : null}
                        </div>
                        <Select
                          value={r.assignee_id ?? "none"}
                          onValueChange={(v) =>
                            updateRow(r._id, { assignee_id: v === "none" ? null : v })
                          }
                        >
                          <SelectTrigger>
                      ...
                      <div>
                        <div className="text-[11px] text-muted-foreground mb-1">
                          Tag{" "}
                          {r.tag_name && !r.tag_id ? (
                            <span className="text-amber-600">({r.tag_name})</span>
                          ) : null}
                        </div>
                        <Select
                          value={r.tag_id ?? "none"}
                          onValueChange={(v) =>
                            updateRow(r._id, { tag_id: v === "none" ? null : v })
                          }
                        >
                          <SelectTrigger>
                            <SelectValue placeholder="—" />
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Select
                        value={r.priority}
                        onValueChange={(v) => updateRow(r._id, { priority: v as Row["priority"] })}
                      >

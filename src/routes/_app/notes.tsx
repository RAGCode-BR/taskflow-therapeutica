...
import { richHtmlToText, sanitizeRichHtml } from "@/lib/sanitize-html";
...
function syncEditorDom(target: HTMLDivElement | null, html: string) {
  ...
  if (!target || target.innerHTML === safeHtml) return;
...
export function NotesWorkspace({
  ...
  const persistNote = async (id: string, patch: Partial<ClientNote>) => {
    const safePatch = patch.content_html === undefined
      ? patch
      : {
          ...patch,
          content_html: sanitizeRichHtml(patch.content_html),
          content: richHtmlToText(patch.content_html),
        };
    if (user && isOffline()) {
      await enqueueOfflineOperation({ userId: user.id, entity: "record", action: "update", entityId: id, payload: { table: "client_notes", patch: safePatch } });
      return;
    }
    const { error } = await sb.from("client_notes").update(safePatch).eq("id", id);
    if (error) toast.error(error.message);
  };
                    ...
                    <li key={n.id} className="flex items-stretch gap-1">
                      ...
                      >
                        ...
                        <p className="mt-1 line-clamp-2 text-[11px] text-muted-foreground">
                          {richHtmlToText(n.content_html || n.content)}
                        </p>
                        {isAdmin && (
...
function NoteEditor({
  ...
  const setEditorsHTML = useCallback((html: string) => {
    ...
    syncEditorDom(editorRef.current, safeHtml);
    syncEditorDom(fsEditorRef.current, safeHtml);
  ...
  const applyEditorChange = useCallback((html: string) => {
    const safeHtml = sanitizeRichHtml(html);
    setEditorsHTML(safeHtml);
    stageChange({ content_html: safeHtml, content: extractTextFromHtml(safeHtml) });
  ...
  const runAIFormat = async () => {
    ...
    try {
      ...
      applyEditorChange(sanitizeRichHtml(res.html));
      ...
      <Card className="mx-auto flex h-full max-w-4xl flex-col overflow-hidden">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-6">
          <div
            ...
            onPaste={onPaste}
            onBlur={() => stageChange({
              content_html: sanitizeRichHtml(editorRef.current?.innerHTML),
              content: richHtmlToText(editorRef.current?.innerHTML),
            })}
      ...
      <Dialog open={fullscreen} onOpenChange={setFullscreen}>
        <DialogContent className="flex h-[96vh] max-h-[96vh] w-[96vw] max-w-[1100px] flex-col gap-3 p-4">
          ...
          <div
            ...
            onPaste={onPaste}
            onBlur={() => stageChange({
              content_html: sanitizeRichHtml(fsEditorRef.current?.innerHTML),
              content: richHtmlToText(fsEditorRef.current?.innerHTML),
            })}

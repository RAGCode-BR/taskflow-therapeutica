  ...
  return (
    <div className="mural-la min-h-full bg-background px-4 py-5 text-[#313532] sm:px-7 sm:py-7">
      <header className="mb-6 flex flex-wrap items-start justify-between gap-4">
        <div>
          <p className="mural-subtle text-[11px] text-[#8a9089]">Mural</p>
                    ...
                    <div className="flex flex-wrap gap-2.5" aria-label="Cor do papel">
                      {COLORS.map((color) => (
                        <button
                          ...
                          className={`h-7 w-7 rounded-full border border-black/10 transition-transform hover:scale-110 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 ${color.card.split(" ")[0]} ${form.color === color.value ? "ring-2 ring-primary ring-offset-2" : ""}`}
                        />
                      ))}
                    </div>
                  </div>
        ...
        <div className="mural-filter flex flex-wrap gap-1 rounded-full bg-white/70 p-1 text-xs shadow-sm">
          {(
            ...
            <button
              ...
              className={`rounded-full px-3 py-1 transition-colors ${postFilter === value ? "bg-primary/15 text-primary" : "text-muted-foreground hover:bg-muted"}`}
            >
              {label}
            </button>
          ))}
      ...
      {isLoading ? (
        ...
        <div className="mural-empty grid min-h-72 place-items-center rounded-xl border border-dashed border-[#d9dcd5] bg-white/50 p-8 text-center">
          <div>
            <div className="mx-auto mb-3 grid h-12 w-12 place-items-center rounded-xl bg-accent text-accent-foreground">
            ...
            {orderedPosts.map((post, index) => {
              ...
              return (
                <article
                  ...
                  className={`group relative flex min-w-0 self-start flex-col rounded-md p-4 shadow-[0_1px_2px_rgba(0,0,0,0.05),0_8px_20px_rgba(0,0,0,0.07)] transition-transform duration-300 hover:-translate-y-0.5 hover:rotate-0 ${index % 3 === 0 ? "-rotate-1" : index % 3 === 1 ? "rotate-1" : "rotate-[0.5deg]"} ${post.card_size === "large" ? "min-h-[220px] md:col-span-2 p-6" : post.card_size === "compact" ? "min-h-[180px] max-w-xs p-3" : "min-h-[180px]"} ${post.is_pinned ? "ring-1 ring-primary/45" : ""} ${draggingId === post.id ? "opacity-75 shadow-xl" : ""} ${colorClass(post.color)}`}
                  style={{
